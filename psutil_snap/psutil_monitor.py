...
if __name__ == "__main__":
    ...
    while(True):
        ...
        data=[]
        data.append({
             '  Max CPU(%) : ' : max_cpu , ' Max Memory(%) : ' : max_mem , ' Total CPU(%) : ': total_cpu_percent, ' Total Memory(%) : ' : total_memory_percent, ' Total Processes : ' : len(processes), 'Sec=' :counter,' || ' : current_time
        })
        results = os.path.expanduser('~/monitor1.csv')
